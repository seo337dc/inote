...
1. ~~**테스트 세팅**~~ — 2026-09-23 완료 (아래 "FE 테스트 작성" 1순위 참고)
2. ~~**만다라트 "E2E 테스트" 항목 수정**~~ — 2026-09-23 완료. `MandalartGrid.tsx` 하드코딩
   라벨 "테스트(E2E)"→"테스트" 수정(커밋 `89d6fd8`), DB t8 축 8개 항목도 FE/BE 테스트+세팅+
   학습+CI로 재구성 완료 (DB 데이터라 별도 커밋 없음, `/dev-guide`에서 브라우저로 반영 확인함)
